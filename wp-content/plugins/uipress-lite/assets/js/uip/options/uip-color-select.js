...
export function moduleData() {
  return {
    ...
    methods: {
      formatValue(value) {
        this.uipress.assignBlockValues(this.colour, value);

        if (typeof value !== 'undefined') {
          if ('gradObject' in value) {
            this.uipress.assignBlockValues(this.gradient, value.gradObject);
          }
        }
    ...
    template: `<div>
        <drop-down dropPos="bottom-left" :onOpen="mountPicker">
            <template v-slot:trigger>
                <div class="uip-background-muted uip-border-round uip-border-right-square uip-padding-xxs">
            ...
            <template v-slot:post-trigger>
                <div class="uip-background-muted uip-padding-xxs uip-border-round uip-border-left-square">
                  <input type="text" class="uip-flex-grow uip-input-small uip-blank-input uip-w-100 " style="line-height:1.2em!important;" v-model="colour.value">
                </div>
            </template>
              ...
              <div class="">
                ...
                <div v-if="ifEnabled('solid')" v-show="mode == 'solid'">
                  ...
                  <div class="uip-flex uip-flex-row uip-gap-xs uip-padding-xs uip-flex-start">
                    ...
                    <div class="uip-border-round uip-border uip-w-28 uip-ratio-1-1" :style="getPreviewBG()">
                    </div>
                  </div>
                
                  <div class="" v-if="ifEnabled('variables')">
                    <div class="uip-padding-xs uip-text-bold">{{strings.themeVariables}}</div>
                    <div class="uip-padding-xs uip-flex uip-flex-row uip-flex-wrap uip-gap-xs uip-row-gap-xs uip-w-240 uip-border-box">
                        <uip-tooltip :message="strings.manageVariables" :delay="500">
                          <div>
                            <div @click="openModal('list-variables', strings.manageVariables)" class="uip-border-circle uip-w-26 uip-h-26 uip-border uip-cursor-pointer uip-flex uip-flex-center uip-flex-middle"><span class=" uip-icon">tune</span></div>
                          </div>
                        </uip-tooltip>
                      <template v-for="item in variables">
                        ...
                        <uip-tooltip v-if="item.type == 'color'"  :message="item.name" :delay="00">
                          <div :class="item.name">
                            <div @click="returnVariable(item)" :data-theme="returnTheme" v-if="item.type = 'color'" class="uip-border-circle uip-w-26 uip-h-26 uip-border uip-cursor-pointer" :style="'background-color:var(' + item.name + ')'"></div>
                          </div>
                        </uip-tooltip>
                      </template>
                    </div>
